...
class DashboardVisual:
    ...
    def obtener_estadisticas(self):
        ...
        try:
            ...
            # Últimas 24 horas
            cursor.execute('''SELECT COUNT(*) FROM anuncios WHERE fecha > datetime(\'now\', \'-24 hours\')''')
            self.estadisticas['ultimas_24h'] = cursor.fetchone()[0]
            
            # Última semana
            cursor.execute('''SELECT COUNT(*) FROM anuncios WHERE fecha > datetime(\'now\', \'-7 days\')''')
    ...
    def dibujar_dashboard(self, estado="esperando", ciclo_info=None):
        ...
        if ultimos_anuncios:
            for i, (titulo, fecha) in enumerate(ultimos_anuncios, 1):
                ...
                try:
                    if 'T' in fecha:
                        # Formato ISO: 2026-04-07T23:28:21.941678
                        fecha_formateada = datetime.fromisoformat(fecha.replace('T', ' ').split('.')[0]).strftime('%H:%M')
                    else:
                        # Formato estándar: 2026-04-07 23:28:21
